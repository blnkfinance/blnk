...
func TestRecordTransaction(t *testing.T) {
	...
	// Start atomic transaction for balance updates AND transaction insert
	mock.ExpectBegin()
...
func TestRecordTransaction_AtomicRollback_InsertFails(t *testing.T) {
	cnf := &config.Configuration{
		Redis: config.RedisConfig{
			Dns: "localhost:6379",
		},
	...
	destination := "destination-balance-id-atomicity-test"
	reference := "transaction-reference-atomicity-test"
	...
	mock.ExpectExec(regexp.QuoteMeta(expectedSQL)).WithArgs(
		...
		sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(),
		destination, sqlmock.AnyArg(), sqlmock.AnyArg(),
		sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(),
	).WillReturnError(fmt.Errorf("database connection lost - simulated failure"))
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to persist transaction with balances")
...
func TestRecordTransaction_AtomicRollback_SecondBalanceUpdateFails(t *testing.T) {
	cnf := &config.Configuration{
		Redis: config.RedisConfig{
			Dns: "localhost:6379",
		},
	...
	destination := "destination-balance-id-atomicity-test-2"
	reference := "transaction-reference-atomicity-test-2"
    ...
    `)).WithArgs(
		...
		sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(),
		sqlmock.AnyArg(), sqlmock.AnyArg(),
	).WillReturnError(fmt.Errorf("optimistic lock failure - balance was modified by another transaction"))
...
func TestRecordTransaction_AtomicRollback_FirstBalanceUpdateFails(t *testing.T) {
	cnf := &config.Configuration{
		Redis: config.RedisConfig{
			Dns: "localhost:6379",
		},
	...
	destination := "destination-balance-id-atomicity-test-3"
	reference := "transaction-reference-atomicity-test-3"
    ...
    `)).WithArgs(
		...
		sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(),
		sqlmock.AnyArg(), sqlmock.AnyArg(),
	).WillReturnError(fmt.Errorf("database write error - disk full"))
...
func TestVoidInflightTransaction_Negative(t *testing.T) {
	...
	t.Run("Transaction not in INFLIGHT status", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta(`SELECT transaction_id, source, reference, amount, precise_amount, precision, currency, destination, description, status, created_at, meta_data, parent_transaction, hash, effective_date FROM blnk.transactions WHERE transaction_id = $1`)).
			WithArgs(transactionID).
			WillReturnRows(sqlmock.NewRows([]string{"transaction_id", "source", "reference", "amount", "precise_amount", "precision", "currency", "destination", "description", "status", "created_at", "meta_data", "parent_transaction", "hash", "effective_date"}).
				AddRow(transactionID, source, gofakeit.UUID(), 100.0, 10000, 100, "USD", destination, gofakeit.UUID(), "APPLIED", time.Now(), metaDataJSON, "", "", nil))
	...
	t.Run("Transaction already voided", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta(`SELECT transaction_id, source, reference, amount, precise_amount, precision, currency, destination, description, status, created_at, meta_data, parent_transaction, hash, effective_date FROM blnk.transactions WHERE transaction_id = $1`)).
			WithArgs(transactionID).
			WillReturnRows(sqlmock.NewRows([]string{"transaction_id", "source", "reference", "amount", "precise_amount", "precision", "currency", "destination", "description", "status", "created_at", "meta_data", "parent_transaction", "hash", "effective_date"}).
				AddRow(transactionID, source, gofakeit.UUID(), 100.0, 10000, 100, "USD", destination, gofakeit.UUID(), "INFLIGHT", time.Now(), metaDataJSON, "", "", nil))
